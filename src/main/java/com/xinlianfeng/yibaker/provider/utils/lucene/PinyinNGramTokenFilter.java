...
public class PinyinNGramTokenFilter extends TokenFilter
{
	...
	{
		this(input, PinyinConstants.DEFAULT_MIN_GRAM, PinyinConstants.DEFAULT_MAX_GRAM, DEFAULT_NGRAM_CHINESE);
	}

	public PinyinNGramTokenFilter(TokenStream input, int maxGram)
	{
		this(input, PinyinConstants.DEFAULT_MIN_GRAM, maxGram, DEFAULT_NGRAM_CHINESE);
	}
	...
	public final boolean incrementToken() throws IOException
	{
		while (true)
		{
			if (this.curTermBuffer == null)
			{
				...
				if ((!this.nGramChinese) && (containsChinese(this.termAtt.toString())))
				{
					return true;
				}
