...
public class PinyinSearchTest
{
	...
	public void test() throws Exception
	{
		...
		String index = filepath.getParent() + File.separator + "index";
		System.out.println(index);
		Directory directory = SimpleFSDirectory.open(Paths.get(index));
		...
		Query query = new TermQuery(new Term(fieldName, "sunyanzi"));
		...
		query = new TermQuery(new Term(fieldName, "孙燕姿"));
		...
		query = new TermQuery(new Term(fieldName, "sunyan"));
		topDocs = searcher.search(query, Integer.MAX_VALUE);
		docs = topDocs.scoreDocs;
		if (null == docs || docs.length <= 0)
		{
